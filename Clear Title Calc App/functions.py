...
class Property_Calc:
  ...
  def property_data(): #function recieving user input for loan, then outputing their results
    ...
    if choice == 'Cash': #if choice is cash, then asks user to input only sales info
      ...
      quote = value_cash * insurance_rate_cash #multiply the customervariable(aka the user's input for price) and multiply it by the global rate to get the rate quote
